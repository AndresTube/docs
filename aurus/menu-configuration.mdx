...
| `on-open` | string list | `[]` | List of [actions](/aurus/actions) to run when the menu finishes opening |
| `on-close` | string list | `[]` | List of [actions](/aurus/actions) to run when the menu closes |
...
<Tip>
Click areas use axis-aligned bounding boxes (AABB). Text components automatically size their hitbox to match the text width. Use `hitbox.width` and `hitbox.height` to override the default hitbox size. When components overlap at different `z` values, the one closest to the camera (lowest `z`) takes priority.
</Tip>
